...
test.describe('Recipes Page - No Hang Test', () => {
  test('should login and verify basic page elements without hanging', async ({ authenticatedPage }) => {
  ...
  test('should test navigation without waiting for content', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/recipes');
    await authenticatedPage.waitForLoadState('domcontentloaded');
  ...
  test('should check page state without hanging', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/recipes');
    await authenticatedPage.waitForLoadState('domcontentloaded');
    ...
    if (hasLoading) {
      console.log('📊 Page shows loading state (expected due to database issues)');
    } else {
