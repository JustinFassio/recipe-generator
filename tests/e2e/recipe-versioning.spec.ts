...
const testRecipe = {
  ...
  instructions: 'Cook quinoa in vegetable broth. Let cool. Mix with vegetables and olive oil. Serve chilled.',
...
const versionUpdate = {
  ...
  changelog: 'Added chickpeas and hemp seeds for extra protein content. Perfect for post-workout meals!'
};
...
test.describe('Recipe Versioning System', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="auth-form"], [data-testid="recipes-page"]', { timeout: 10000 });
    
    // Check if we need to sign in
    const isSignedIn = await page.locator('[data-testid="recipes-page"]').isVisible();
    ...
    if (!isSignedIn) {
      ...
      await page.waitForSelector('[data-testid="recipes-page"]', { timeout: 10000 });
    }
  });

  test('Complete versioning workflow: Create → Version → Rate → Explore', async ({ page }) => {
    ...
    for (let i = 0; i < testRecipe.ingredients.length; i++) {
      ...
      await page.fill(`input[placeholder*="Ingredient ${i + 1}"]`, testRecipe.ingredients[i]);
    }
    
    // Fill instructions
    await page.fill('textarea[placeholder*="cooking instructions"]', testRecipe.instructions);
    ...
    if (await createVersionBtn.isVisible()) {
      ...
      await page.fill('input[placeholder*="version"]', versionUpdate.versionName);
      await page.fill('textarea[placeholder*="changed"]', versionUpdate.changelog);
      ...
      console.log('⚠️ Create Version button not visible - might be version system not fully loaded');
    }
    ...
    if (await exploreRecipeCard.isVisible()) {
      ...
      console.log('⚠️ Recipe not found in Explore - might need more time for indexing');
    }
  ...
  test('Recipe view page version navigation', async ({ page }) => {
    ...
    if (await firstRecipe.isVisible()) {
      ...
      const versionButtons = page.locator('button:has-text("View Versions"), button:has-text("Versions")');
  ...
  test('Explore page versioning features', async ({ page }) => {
    ...
    const recipeCards = page.locator('[data-testid="recipe-card"], .card').first();
    ...
    if (await recipeWithVersions.isVisible()) {
      ...
      if (await versionsButton.isVisible()) {
        ...
        if (await rateButtons.first().isVisible()) {
          ...
          const submitButton = page.locator('button:has-text("Submit"), button:has-text("Rate")');
  ...
  test('Database migration validation', async ({ page }) => {
    ...
    if (await versionSorting.isVisible() && await topRatedSorting.isVisible()) {
      console.log('✅ Version-aware sorting available - migration successful');
    } else {
      console.log('⚠️ Version-aware sorting not found - migration may not be applied');
    }
  ...
  test('Error handling and edge cases', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      const emptyState = page.locator('text*=No recipes, text*=no results, text*=not found');
