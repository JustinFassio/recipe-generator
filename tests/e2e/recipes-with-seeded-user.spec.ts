...
test.describe('Recipes Page with Seeded User', () => {
  test('should login as Alice and see her recipes', async ({ authenticatedPage }) => {
  ...
  test('should find Alice\'s specific recipes', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/recipes');
    await authenticatedPage.waitForLoadState('networkidle');
    ...
    if (hasAvocadoToast) {
      ...
      await authenticatedPage.screenshot({ path: 'alice-recipes-not-found.png' });
    }
  });

  test('should test recipe page navigation and buttons', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/recipes');
    await authenticatedPage.waitForLoadState('networkidle');
    ...
    const filtersButton = authenticatedPage.getByRole('button', { name: /filters.*search/i });
    await expect(filtersButton).toBeVisible();
    
    console.log('✅ All navigation buttons are visible and working');
  });

  test('should test adding a new recipe workflow', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/recipes');
    await authenticatedPage.waitForLoadState('networkidle');
    ...
    const ingredientsTextarea = authenticatedPage.getByRole('textbox', { name: /ingredients/i });
    const instructionsTextarea = authenticatedPage.getByRole('textbox', { name: /instructions/i });
  ...
  test('should test AI recipe creator workflow', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/recipes');
    await authenticatedPage.waitForLoadState('networkidle');
