...
test.describe('Recipes Page Review', () => {
  test('should login and review recipes page elements', async ({
    authenticatedPage,
  }) => {
  ...
  test('should find and interact with recipe page buttons', async ({
    authenticatedPage,
  }) => {
    await authenticatedPage.goto('/recipes');
    await authenticatedPage.waitForLoadState('networkidle');
    ...
    const searchBox = authenticatedPage.getByRole('textbox', {
  ...
  test('should find filter buttons on recipes page', async ({
    authenticatedPage,
  }) => {
    await authenticatedPage.goto('/recipes');
    await authenticatedPage.waitForLoadState('networkidle');
    ...
    const categoriesButton = authenticatedPage.getByRole('button', {
      name: /categories/i,
    });
    await expect(categoriesButton).toBeVisible();

    const cuisinesButton = authenticatedPage.getByRole('button', {
      name: /cuisines/i,
    });
    await expect(cuisinesButton).toBeVisible();

    const moodsButton = authenticatedPage.getByRole('button', {
      name: /moods/i,
    });
    await expect(moodsButton).toBeVisible();

    const ingredientsButton = authenticatedPage.getByRole('button', {
      name: /ingredients/i,
    });
  ...
  test('should handle empty recipes state', async ({ authenticatedPage }) => {
    ...
    const addFirstRecipeButton = authenticatedPage.getByRole('button', {
      name: /add your first recipe/i,
    });
