...
test.describe('Recipes Page', () => {
  test('should display recipes page for authenticated user', async ({
    authenticatedPage,
  }) => {
    ...
    await expect(
      authenticatedPage.getByRole('heading', { name: /recipes/i })
    ).toBeVisible();
  });

  test('should allow user to create new recipe', async ({
    authenticatedPage,
  }) => {
    // Look for "Add Recipe" or "Create Recipe" button
    const addRecipeButton = authenticatedPage.getByRole('button', {
      name: /add|create.*recipe/i,
    });
  ...
  test('should allow user to view recipe details', async ({
    authenticatedPage,
  }) => {
    // Wait for recipes to load
    await authenticatedPage.waitForLoadState('networkidle');
    ...
    const firstRecipe = authenticatedPage
      .locator('[data-testid="recipe-card"]')
      .first();
    const recipeLink = authenticatedPage
      .getByRole('link', { name: /recipe/i })
      .first();
...
test.describe('Recipe Creation', () => {
  test('should create a new recipe', async ({ authenticatedPage }) => {
    ...
    await authenticatedPage
      ...
      .fill('A test recipe for E2E testing');

    // Add ingredients
    const ingredientInput = authenticatedPage.getByRole('textbox', {
      name: /ingredient/i,
    });
    ...
    // Add instructions
    const instructionInput = authenticatedPage.getByRole('textbox', {
      name: /instruction|step/i,
    });
