...
test.describe('Profile Management', () => {
  test.describe('Profile Page Access', () => {
    test('should access profile page for authenticated user', async ({
      authenticatedPage,
    }) => {
  ...
  test.describe('Profile Information Updates', () => {
    test('should update full name', async ({ authenticatedPage }) => {
      ...
      const fullNameInput = authenticatedPage.getByRole('textbox', {
        name: /full name/i,
      });
      ...
      await expect(authenticatedPage.getByText(/success|updated/i)).toBeVisible(
        { timeout: 10000 }
      );
    });
    ...
    test('should update bio', async ({ authenticatedPage }) => {
      ...
      const bioTextarea = authenticatedPage.getByRole('textbox', {
        name: /bio/i,
      });
      ...
      // Save the bio
      const saveBioButton = authenticatedPage.getByRole('button', {
        name: /save.*bio/i,
      });
      await saveBioButton.click();

      // Wait for success message
      await expect(authenticatedPage.getByText(/success|saved/i)).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('should update location information', async ({
      ...
      // Update country
      const countrySelect = authenticatedPage.getByRole('combobox', {
        name: /country/i,
      });
      ...
      // Update city
      const cityInput = authenticatedPage.getByRole('textbox', {
        name: /city/i,
      });
    ...
    test('should update cooking preferences', async ({ authenticatedPage }) => {
      ...
      const cookingSection =
        authenticatedPage.getByText(/cooking|preferences/i);
      if ((await cookingSection.count()) > 0) {
        // Update skill level
        const skillLevelSelect = authenticatedPage.getByRole('combobox', {
          name: /skill.*level/i,
        });
        if ((await skillLevelSelect.count()) > 0) {
          await skillLevelSelect.click();
          await authenticatedPage
            .getByRole('option', { name: /intermediate/i })
            .click();
        }

        // Update time per meal
        const timeSlider = authenticatedPage.getByRole('slider', {
          name: /time.*meal/i,
        });
        ...
        const saveCookingButton = authenticatedPage.getByRole('button', {
          name: /save.*cooking/i,
        });
        if ((await saveCookingButton.count()) > 0) {
          await saveCookingButton.click();
          await expect(
            authenticatedPage.getByText(/success|saved/i)
    ...
    test('should update safety preferences', async ({ authenticatedPage }) => {
      ...
      const safetySection = authenticatedPage.getByText(
        /allergies|dietary|safety/i
      );
      if ((await safetySection.count()) > 0) {
        // Add an allergy
        const allergyInput = authenticatedPage.getByRole('textbox', {
          name: /allerg/i,
        });
        ...
        const dietaryInput = authenticatedPage.getByRole('textbox', {
          name: /dietary/i,
        });
        ...
        const saveSafetyButton = authenticatedPage.getByRole('button', {
          name: /save.*safety/i,
        });
  ...
  test.describe('Profile Data Persistence', () => {
    test('should persist profile changes after page refresh', async ({
      authenticatedPage,
    }) => {
      await authenticatedPage.goto('/profile');
    ...
    test('should maintain profile data across navigation', async ({
      authenticatedPage,
    }) => {
      await authenticatedPage.goto('/profile');
  ...
  test.describe('Profile Validation', () => {
    test('should validate full name length', async ({ authenticatedPage }) => {
      ...
      await expect(
        authenticatedPage.getByText(/80 characters or less/i)
      ).toBeVisible();
    });
    ...
    test('should validate username format', async ({ authenticatedPage }) => {
      ...
      if ((await usernameInput.count()) > 0) {
        ...
        // Check for validation error
        await expect(
          authenticatedPage.getByText(/invalid|format/i)
        ).toBeVisible();
      }
    });
  });
  ...
  test.describe('Profile Loading States', () => {
    test('should show loading state during profile update', async ({
      ...
      await expect(
        authenticatedPage.getByText(/loading|saving/i)
      ).toBeVisible();
    });
  ...
  test.describe('Profile Avatar', () => {
    test('should display avatar upload section', async ({
      authenticatedPage,
    }) => {
      await authenticatedPage.goto('/profile');
      ...
      const avatarSection =
        authenticatedPage.getByText(/avatar|photo|picture/i);
    ...
    test('should show avatar upload button', async ({ authenticatedPage }) => {
      ...
      const uploadButton = authenticatedPage.getByRole('button', {
        name: /upload|change.*avatar/i,
      });
