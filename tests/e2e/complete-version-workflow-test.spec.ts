...
test.describe('Complete Version Workflow Test', () => {
  ...
  test('Alice Baker complete version creation workflow', async ({ page }) => {
    console.log('🎯 Testing complete version creation workflow for Alice Baker...');
    ...
    const zucchiniHeading = page.getByRole('heading', { name: 'Zucchini Noodles with Pesto', level: 3, exact: true });
    await expect(zucchiniHeading).toBeVisible({ timeout: 10000 });

    // Click the recipe menu and edit recipe
    const zucchiniCard = page.locator('div').filter({ hasText: 'Zucchini Noodles with Pesto' }).first();
    const menuButton = zucchiniCard.locator('label[aria-label="Recipe actions"]').first();
    ...
    if (await setupSection.isVisible()) {
      ...
      const addSetupButton = setupContainer.getByRole('button', { name: /add.*step/i });
      ...
      if (await addSetupButton.isVisible()) {
        ...
        if (setupInputCount > 0) {
          ...
          console.log('✅ Added setup item: "Prepare ice water bath for blanching"');
        }
      }
    } else {
    ...
    if (await ingredientsSection.isVisible()) {
      ...
      const ingredientsContainer = ingredientsSection.locator('..').locator('..');
      const addIngredientButton = ingredientsContainer.getByRole('button', { name: /add.*ingredient/i });
    ...
    const instructionsTextarea = page.getByRole('textbox', { name: /instructions/i });
    if (await instructionsTextarea.isVisible()) {
      const currentInstructions = await instructionsTextarea.inputValue();
      const updatedInstructions = currentInstructions + ' Finish with a sprinkle of fresh lemon zest for extra brightness.';
    ...
    const saveAsVersionButton = page.getByRole('button', { name: /save.*new.*version/i });
    if (await saveAsVersionButton.isVisible()) {
      await saveAsVersionButton.click();
      await page.waitForTimeout(1000);
      
      // Fill in version details modal
      const versionNameInput = page.getByRole('textbox', { name: /version name/i });
      ...
      if (await changelogInput.isVisible()) {
        await changelogInput.fill('Added fresh lemon zest and ice water bath setup for enhanced flavor and texture');
        console.log('✅ Filled changelog');
      }
      
      // Submit the version
      const createVersionButton = page.getByRole('button', { name: /create/i }).last();
      ...
      console.log(`🧊 Recipe content includes "ice water bath": ${hasIceWaterBath}`);
      
      // Step 9: Check if version navigation is available
      console.log('🔍 Step 9: Checking version navigation');
      
      const viewVersionsButton = page.getByRole('button').filter({ hasText: /view.*versions/i });
      ...
      if (hasVersionsButton) {
        ...
        if (await modal.isVisible()) {
          ...
          // Count version cards
          const versionCards = modal.locator('div').filter({ hasText: /Version \d+/ });
          ...
          console.log(`🍋 "Zesty Lemon" version visible in modal: ${hasZestyVersion}`);
      ...
      const workflowSuccess = hasVersionParam && (hasLemonZest || hasIceWaterBath) && hasVersionsButton;
      console.log(`🎯 Complete workflow success: ${workflowSuccess ? '✅' : '❌'}`);
      
      if (workflowSuccess) {
        console.log('🎉 SUCCESS: Version creation workflow is working correctly!');
        ...
        console.log(`  - Content changes: ${hasLemonZest || hasIceWaterBath ? '✅' : '❌'}`);
        console.log(`  - Version navigation: ${hasVersionsButton ? '✅' : '❌'}`);
      }
    ...
    await page.screenshot({ path: 'complete-version-workflow.png', fullPage: true });
