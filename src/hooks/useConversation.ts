...
export function useConversation(): ConversationState & ConversationActions {
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      let errorContent = "I'm sorry, I'm having trouble connecting right now. Could you please try sending your message again?";
      
      if (error instanceof Error) {
        if (error.message.includes('Rate limit') || error.message.includes('Too many requests')) {
          errorDescription = 'Rate limit reached. Please wait 30 seconds before trying again.';
          errorContent = "I'm getting too many requests right now. Please wait about 30 seconds and then try your message again. Thanks for your patience!";
        } else if (error.message.includes('API key')) {
          errorDescription = 'API configuration issue. Please check your OpenAI settings.';
          errorContent = "There seems to be an issue with my configuration. Please contact support if this continues.";
        } else if (error.message.includes('Access denied')) {
          errorDescription = 'Access denied. Please check your OpenAI account permissions.';
          errorContent = "I don't have permission to access the AI service right now. Please contact support.";
        } else if (error.message.includes('Assistant') || error.message.includes('thread') || error.message.includes('timeout')) {
          errorDescription = 'Assistant service temporarily unavailable. Using standard chat instead.';
          errorContent = "I'm having trouble with my advanced features right now, but I can still help you with recipes using my standard capabilities!";
        }
      }
  ...
  const startNewRecipe = useCallback(() => {
    ...
    if (shouldConfirm) {
      const ok = window.confirm('Start a new recipe? This will clear the current conversation.');
      if (!ok) return;
    }
  ...
  const convertToRecipe = useCallback(async () => {
    if (!persona || messages.length < 2) {
      toast({
        ...
        description: 'Please have a conversation about a recipe before trying to save.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('format')) {
          errorDescription = 'Could not parse the recipe format. The AI response may not contain a complete recipe.';
        } else if (error.message.includes('No recipe content')) {
          errorDescription = 'No recipe found in the conversation. Try asking the AI to provide a complete recipe.';
        }
      }
