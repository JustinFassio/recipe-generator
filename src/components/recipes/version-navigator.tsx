...
export function VersionNavigator({
  ...
  const loadVersions = async () => {
    try {
      ...
      console.log(`🔍 [VersionNavigator] Loading versions for recipe: ${recipeId}`);
      ...
      console.log(`📊 [VersionNavigator] Loaded ${versionData.length} versions`);
      ...
      <div className="space-y-3">
        {versions.map((version, index) => (
          <VersionCard
            ...
            onSelect={() => {
              console.log(`🔄 [VersionNavigator] Selected version ${version.version_number}`);
    ...
    >
      ...
      {version.changelog && (
        <div className="text-sm text-gray-600 mb-2">
          <strong>{isOriginal ? 'Description:' : 'Changes:'}</strong> {version.changelog}
        </div>
      )}
      
      <div className="text-xs text-gray-500 flex items-center justify-between">
        <span>
          {isOriginal ? 'Original recipe' : `Created ${new Date(version.created_at).toLocaleDateString()}`}
        </span>
