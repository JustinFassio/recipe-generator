    ...
    if (loading) {
      ...
      loadingTimeoutRef.current = setTimeout(() => {
        if (isMountedRef.current && loading) {
          logger.error(
            'Loading timeout - forcing navigation to sign-in after 30s'
          );
          logger.error('Auth state at timeout:', {
            user: !!user,
            loading,
            error,
          ...
          {import.meta.env.DEV && (
            <div className="mt-8 p-4 bg-base-200 rounded-lg text-left max-w-md mx-auto">
              ...
              <div className="text-xs space-y-1 text-base-content/80">
                ...
                <div>
                  Requires Auth: <code>{requiresAuth ? 'true' : 'false'}</code>
                </div>
                <div>
                  Mounted:{' '}
                  <code>{isMountedRef.current ? 'true' : 'false'}</code>
                </div>
                <div>
                  Timeout Active:{' '}
                  <code>{loadingTimeoutRef.current ? 'true' : 'false'}</code>
                </div>
              </div>
              <button
