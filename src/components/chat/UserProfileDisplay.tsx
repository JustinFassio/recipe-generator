...
export const UserProfileDisplay: React.FC<UserProfileDisplayProps> = ({
  ...
  return (
    <div
      className={`bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 p-4 rounded-lg ${className}`}
    >
        ...
        <div className="space-y-2">
          ...
          {userData.safety.dietary_restrictions.length > 0 && (
            <div className="ml-6">
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {userData.safety.dietary_restrictions.map(
                  (restriction, index) => (
                    <span
                      key={index}
            ...
            {userData.profile.time_per_meal && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600">
                  Time:{' '}
                  <span className="font-medium">
                    {userData.profile.time_per_meal} min
                  </span>
                </span>
              </div>
            )}
            {userData.cooking.preferred_cuisines.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600">
                  Cuisines:{' '}
                  <span className="font-medium">
                    {userData.cooking.preferred_cuisines.join(', ')}
                  </span>
                </span>
              </div>
            )}
            ...
            <div className="text-xs text-orange-600 space-y-1">
              {liveSelections.categories.length > 0 && (
                <p>
                  ...
                  {liveSelections.categories.join(', ')}
                </p>
              )}
              {liveSelections.cuisines.length > 0 && (
                <p>
                  🌍 <strong>Cuisines:</strong>{' '}
                  {liveSelections.cuisines.join(', ')}{' '}
                  <span className="text-orange-500">
                    (Override account preferences)
                  </span>
                </p>
              )}
              {liveSelections.moods.length > 0 && (
                <p>
                  💫 <strong>Moods:</strong> {liveSelections.moods.join(', ')}
                </p>
              )}
              <p className="mt-2 font-medium">
                These selections override your account preferences for this
                specific meal!
              </p>
            </div>
          </div>
      ...
      <div className="mt-3 pt-3 border-t border-blue-200">
        <p className="text-xs text-blue-600">
          💡 All AI assistants now have access to your profile and will provide
          personalized, safe recommendations!
        </p>
      </div>
    </div>
  );
