...
export function EnvDebug() {
  ...
  const hasLocalhost = envInfo.VITE_SUPABASE_URL?.includes('127.0.0.1') || envInfo.VITE_SUPABASE_URL?.includes('localhost');

  return (
    <div className="fixed bottom-4 right-4 card bg-base-100 shadow-xl border border-error">
      <div className="card-body p-4 max-w-md">
        ...
        <div className="space-y-2 text-xs">
          {Object.entries(envInfo).map(([key, value]) => (
            <div key={key} className="flex justify-between">
              ...
              <span className={`font-mono ${value === 'NOT SET' ? 'text-error' : 'text-success'}`}>
                {value}
              </span>
            </div>
          ))}
        ...
        {hasLocalhost && (
          <div className="mt-4 p-3 bg-base-200 rounded-lg">
            <h4 className="font-bold text-sm mb-2">How to Fix:</h4>
