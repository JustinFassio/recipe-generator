  ...
  try {
    ...
    if (!/^[a-z0-9_]{3,24}$/.test(cleanUsername)) {
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: 'Username must be 3-24 characters long and contain only lowercase letters, numbers, and underscores' 
        }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    ...
    if (reservedError && reservedError.code !== 'PGRST116') { // PGRST116 = no rows returned
      console.error('Error checking reserved usernames:', reservedError)
      return new Response(
        JSON.stringify({ success: false, error: 'Database error checking reserved usernames' }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    if (error) {
      ...
      if (error.message?.includes('username_already_taken')) {
        return new Response(
          JSON.stringify({ success: false, error: 'This username is already taken' }),
          { 
            status: 409, 
            headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
          }
      ...
      if (error.message?.includes('user_already_has_username')) {
        return new Response(
          JSON.stringify({ success: false, error: 'You already have a username' }),
          { 
            status: 400, 
            headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
          }
